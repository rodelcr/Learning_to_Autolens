...
|---|---|---|---|---|
| 1 | **[compound_lens/](compound_lens/)** | Two deflectors at different z, one source | ✓ shipped + [climb] climb | Multi-plane `al.Tracer`, staged chaining across lens bodies, when to simplify to an effective deflector |
| 2 | **[double_source_plane/](double_source_plane/)** | One lens, two sources at different z | ✓ shipped + [climb] climb | Cosmological distance ratios, joint multi-source posterior, β cosmography |
| 3 | [group_scale/](group_scale/) | BGG + satellites + extended envelope (same z) | ◐ in-progress + [climb] climb | Cluster-lite multi-component mass; freely-fit attempts stall but `truth_anchored` PASSED (χ²/N=1.025, max\|res\|=4.50σ) — diagnostic confirms search-space exploration is the bottleneck, not model representability (see `group_scale/01_group_scale_fit.ipynb` §3.1) |
...
| 7 | [compound_lens_zoo/](compound_lens_zoo/) | Five compound mocks (mock_2–mock_6, lenstronomy origin) — same prior set, varying z, γ′, cosmology | ◐ in-progress + [climb] climb | Methodology robustness; cosmology mis-specification effects on recovered (θ_E, γ′); the **production climb** at scale (R0→R5 ladder) |
...
| 11 | [cluster_scale/](cluster_scale/) | BCG + 10 cluster members + 2 sources at different z | ✓ shipped (truth_anchored 2026-05-04 χ²/N=1.006, max\|res\|=4.04σ — strict-PASS) | Architectural union of group_scale + double_source_plane; Faber-Jackson scaling for member masses (1 free param shared); member SersicSph light tightly anchored on truth; multi-source $\beta_{ij}$ cosmography is the v0.96 extension via `cosmography_joint_posterior` |
| 12 | **[galaxy_galaxy_single_arc/](galaxy_galaxy_single_arc/)** | SLACS-style single deflector + single arc (the canonical first fit) | ✓ shipped (v0.95, ggsa_direct χ²/N=0.957, max\|res\|=3.88σ — strict-PASS) | The missing pedagogical starting point; what "converged" looks like before tackling compound systems |
| 13 | **[radial_arc_smbh/](radial_arc_smbh/)** | Einstein-spiral analog + embedded SMBH (PowerLaw + PointMass) | ✓ shipped (v0.96, rarc_direct + with_pointmass both strict-PASS; Δlog_Z=+1.71 — BH NOT imaging-detected, γ′–M_BH degeneracy demonstrated) | The AGEL Einstein-spiral methodology bridge; kinematics-break unlocked via `--part=with_kinematics` (Phase 3 `_jeans_sigma_v.py`) |
| 14 | **[cosmography_joint_posterior/](cosmography_joint_posterior/)** | DSPL × TDCOSMO joint cosmography (Birrer+2020 §4) | ✓ shipped (v0.96, laptop-only notebook on landed chains) | Combines β-cosmography (Ωₘ, w₀) from DSPL with H₀ from TDCOSMO; v0.97 will do full FactorGraphModel single-likelihood combination |
| 15 | [positions_modeling/](positions_modeling/) | Position-likelihood tutorial (pedagogy on `al.PositionsLH`) | ✓ shipped (v0.95) | When and how to add position-likelihood priors; reuses compound_lens + quad_time_delay results |

**Status legend.** ✓ shipped: at least one converged Cannon result committed and audited, OR (for pedagogy-only examples) the notebook builds cleanly on existing committed results. ◐ in-progress: notebook + driver exist, results not yet finalised. [climb] climb: a `00_climb_to_*.ipynb` bridge notebook is available, teaching the iterative-masking + position-likelihood techniques in the context of this architecture.
