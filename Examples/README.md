...
| 11 | [cluster_scale/](cluster_scale/) | BCG + 10 cluster members + 2 sources at different z | ✓ shipped (truth_anchored 2026-05-04 χ²/N=1.006, max\|res\|=4.04σ — strict-PASS) | Architectural union of group_scale + double_source_plane; Faber-Jackson scaling for member masses (1 free param shared); member SersicSph light tightly anchored on truth; multi-source $\beta_{ij}$ cosmography is the v0.93+ extension |

**Status legend.** ✓ shipped: at least one converged Cannon result committed and audited. ◐ in-progress: notebook + driver exist, results not yet finalised. [climb] climb: a `00_climb_to_*.ipynb` bridge notebook is available, teaching the iterative-masking + position-likelihood techniques in the context of this architecture.
