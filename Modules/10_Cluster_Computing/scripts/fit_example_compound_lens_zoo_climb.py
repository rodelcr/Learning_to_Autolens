...
ladder beyond R2 (the canonical zoo fit). Implements R3 (multi-plane) and
R2_2src (single-plane + 2-source) per the ladder analysis in
Examples/compound_lens_zoo/02_compound_lens_ladder.ipynb.
...
Usage:
    python fit_example_compound_lens_zoo_climb.py --rung R3 --mock 3 \\
...
def main():
    ...
    p.add_argument("--rung", choices=["R3", "R2_2src"], required=True)
    ...
    # Defaults per rung: R3 -> mocks 3, 4 (the multi-plane signal mocks);
    # R2_2src -> mock 6 (the single-deflector + 2-source mock).
    if args.mock == "all":
        if args.rung == "R3":
            mocks_to_fit = [3, 4]
        else:
